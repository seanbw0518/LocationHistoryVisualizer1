...
class Point():
    def __init__(self, coordinate, timestamp, accuracy):
...
def extract_coordinates(json_data):
    ...
    num_of_coords = len(json_data["semanticSegments"])
    print("Found " + str(num_of_coords) + " semanticSegments, extracting...")
    ...
    # Parse through all semantic segments
    for segment in json_data.get('semanticSegments', []):
        # Only process segments that have "timelinePath" or "visit"
        if 'timelinePath' in segment:
            for pathCoord in segment['timelinePath']:

                point = [coord.replace('Â°', '') for coord in pathCoord['point'].split(',')]
                point[0] = float(point[0])
                point[1] = float(point[1])

                parsed_coord_points.append({
                    'point': point,
                    'time': pathCoord['time'],
                    'accuracy': 1
                })
        elif 'visit' in segment:
            point = [coord.replace('Â°', '') for coord in segment['visit']['topCandidate']['placeLocation']['latLng'].split(',')]
            point[0] = float(point[0])
            point[1] = float(point[1])
            ...
            parsed_coord_points.append({
                ...
                'accuracy': segment['visit']['topCandidate']['probability']
            })
        elif 'activity' in segment:
            startPoint = [coord.replace('Â°', '') for coord in segment['activity']['start']['latLng'].split(',')]
            startPoint[0] = float(startPoint[0])
            startPoint[1] = float(startPoint[1])

            endPoint = [coord.replace('Â°', '') for coord in segment['activity']['end']['latLng'].split(',')]
            endPoint[0] = float(endPoint[0])
            endPoint[1] = float(endPoint[1])

            accuracy = 0
            if ('probability' in segment['activity']):
                accuracy = segment['activity']['probability']
    ...
    #last_coord = (json_data["semanticSegments"][0]["latitudeE7"]/ 10000000.0, json_data["locations"][0]["longitudeE7"]/ 10000000.0)
    last_coord = parsed_coord_points[0]['point']
    last_time = parsed_coord_points[0]['time']
    ...
    for item in parsed_coord_points:
        ...
        try:
            ...
            if dist_diff < distance_threshold and time_diff < time_threshold:
                new_point = Point(this_coord, this_time, accuracy)
    ...
    # remove any sets with a minimum number of coordinates in it
...
# Open the JSON file
print("Opening Timeline.json, this can take a while...")
